---
title: Overcoming Python GIL with Cython
date: 2013-12-23
tags:
- programming
- python
...
As explained very well in Beazley\'s lesson, the threaded version suffers from a very bad behavior where the OS tries desperately to schedule the two threads on different cores, but, because of the GIL, only one runs at a time whereas the other is moved across cores endlessly...

In fact, a better behavior is achievable forcing the OS to run the script using just one CPU, using `tasksel`. This takes as long as the serial version:
...
There is a way to overcome the limitation imposed by the GIL for CPU bound threads though: using [Cython](http://docs.cython.org/index.html). [Cython](http://docs.cython.org/index.html) offers a wonderful context manager to run instructions without the GIL: [with nogil](http://docs.cython.org/src/userguide/external_C_code.html?highlight=nogil#acquiring-and-releasing-the-gil). The catch is that it can only be used to run code that does not touch any Python object.
