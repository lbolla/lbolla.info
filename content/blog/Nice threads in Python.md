...
Take a look at [this script](https://gist.github.com/lbolla/699c44601ac0bd50d1147bf08931d9f1):
...
All threads terminate at the same time (they are given the same amount of CPU time), but they all take 13 secs, to execute a task (`fib(26)`) that takes \~3 secs to execute, because threads are stepping on each other\'s toes.
...
If instead each thread is given a different priority, things behave better. We can do that by using [nice](https://linux.die.net/man/1/nice). The output looks like this:
...
Note that the threads terminate at different times: the OS scheduled more CPU-time to the high priority thread, which completed before the others. [`htop` shows it in \"real time\"](https://youtu.be/iVdYAY4mnr0), too:
